...
class NodeSelector:
    ...
    async def select_node(self, 
        ...
        for node in nodes:
            logger.debug(f"  - {node.node_id[:8]}... at {node.url or 'local'} (model: {node.model}, load: {node.load})")
        ...
        if selected:
            logger.info(f"🎯 Selected node {selected.node_id[:8]}... at {selected.url or 'local'} (model: {selected.model}) via {strategy} strategy")
